...
// ListDrivers artık pointer ile nil gönderebileceğimiz FindByParams üzerinden yapılabilir
func (s *DriverService) ListDrivers(page, pageSize int, ctx context.Context) ([]*entities.Driver, error) {
	return s.repo.FindByParams(nil, nil, nil, nil, nil, &page, &pageSize, ctx)
}

// --------------------
// Yakındaki Taksileri Bulma
// --------------------
func (s *DriverService) GetNearbyDrivers(lat, lon float64, taxiType string, ctx context.Context) ([]*dtos.DriverResponseDTO, error) {
	...
	// FindByParams pointer parametreleri alıyor
	minLatPtr := &minLat
	maxLatPtr := &maxLat
	minLonPtr := &minLon
	maxLonPtr := &maxLon
	...
	drivers, err := s.repo.FindByParams(minLatPtr, maxLatPtr, minLonPtr, maxLonPtr, taxiTypePtr, nil, nil, ctx)
	if err != nil {
		return nil, err
	}

	// Haversine ile gerçek mesafeyi hesapla ve 6 km’den uzakları eleriz
